...
class BUTDFCNet(nn.Module):
    def __init__(self, in_shape: List[int], shared_weights: bool = True, **kwargs):
        ...
        fc_shapes = [fc_in_shape] + [self.out_shape]*depth

        self._fc_layers = nn.ModuleList([
            BUTDLinear(fc_shapes[i], fc_shapes[i+1], shared_weights=shared_weights, **kwargs)
            for i in range(depth)
        ])

        self.layers = BUTDSequential(self._flat, *self._fc_layers)
