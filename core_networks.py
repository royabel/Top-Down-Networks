...
class BUTDSimpleNet(nn.Module):
    """
    Simple network architecture based on Le-Net.
    Pooling layers are replaced with strided convolution layers.
    """
    def __init__(self, in_shape: List[int], shared_weights: bool = True, **kwargs):
        ...
        # Calculates the spatial shape returned from the conv layers to infer the input shape of the following layer
        if len(in_shape) == 3:
...
class BUTDConvNet(nn.Module):
    """
    Basic convolution network. Starts with multiple convolution layers followed by fully connected layers.
    """
...
class BUTDResNet(nn.Module):
    """
    Similar to Pytorch ResNet implementation, but replaces standard layer with their corresponding BUTD layers.
    """
