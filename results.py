...
def _generate_comparison_plot(results_dict, agg='mean', key_filter=None, exclude=None, const_val=None, start_idx=0,
                              sym_vs_asym=True, suptitle=None, save=False):
    ...
    for i, m in enumerate(metrics_list):
        for k, v in results_dict.items():
            ...
            # axs[i // 2, i % 2].plot(v[agg][m][start_idx:], c=c_map[_file_to_suffix(k)])
            axs[i // 2, i % 2].set_xlabel('Epoch')
            axs[i // 2, i % 2].set_ylabel(m)
        ...
        if const_val is not None and i==2:
        ...
        if i == 0 :
            axs[i // 2, i % 2].legend(prop={'size': 12})
    ...
    if save:
        plt.savefig(suptitle, bbox_inches='tight')
    else:
        plt.show()
...
def _get_res_dict(directory_path):
    ...
    for filename in os.listdir(directory_path):
        ...
        res_dict[filename[:-4]] = {
            ...
            'confident': results_df.groupby(['epoch']).agg(np.std)
...
if __name__ == '__main__':
    path_dir = 'runs/Cifar Resnet'
    res_dict = _get_res_dict(path_dir)

    _generate_comparison_plot(res_dict, key_filter=[], start_idx=0, exclude=['0_01', 'Noisy'], sym_vs_asym=False,
                              suptitle=None)
